...
class Pengeluaran extends CI_Controller {
	public function __construct()
	{
		...
		$this->data['url'] = site_url('laporan/pengeluaran');  
	...
	public function index() { 
		...
		$this->data['title'] = 'Laporan Pengeluaran Masjid';  
	 ...
	 public function report($bln){
		...
		$cRet .="<table width='100%' style='font-size:14px; border-collapse:collapse; font-family:arial;' border='0'>
					<tr>
						...
						<td align='center' width='60%'><b>PENGELUARAN MASJID</b></td>
		...
		";
		$list = $this->pengeluarans->get_report($bln);
		$total = 0;
		foreach($list as $row){ 
			$cost = number_format($row->kredit);
